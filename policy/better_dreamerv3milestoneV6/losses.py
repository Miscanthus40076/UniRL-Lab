...
def compute_slow_gain_reward_target(
    ...
    del batch, config
    return outputs.get("interest_reward_components")
...
def world_model_loss(outputs: dict, batch: dict, config: WorldModelLossConfig) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    if slow_gain_reward_components is not None:
        raw_slow_gain_reward = slow_gain_reward_components["raw_slow_gain_reward"]
        interest_slow_gain_reward = slow_gain_reward_components["interest_slow_gain_reward"]
        interest_weight = slow_gain_reward_components["interest_weight"]
        interest_evidence_feat = slow_gain_reward_components["interest_evidence_feat"]
        residual_mag_feat = slow_gain_reward_components["residual_mag_feat"]
        positive_gain_feat = slow_gain_reward_components["positive_gain_feat"]
        ...
        metrics["wm/raw_slow_gain_reward_mean"] = raw_slow_gain_reward.mean()
        metrics["wm/slow_gain_reward_mean"] = interest_slow_gain_reward.mean()
        metrics["wm/interest_slow_gain_reward_mean"] = interest_slow_gain_reward.mean()
        metrics["wm/slow_gain_reward_std"] = interest_slow_gain_reward.std(unbiased=False)
        metrics["wm/slow_gain_reward_nonzero_rate"] = (interest_slow_gain_reward > 0).float().mean()
        metrics["wm/interest_slow_gain_reward_nonzero_rate"] = (interest_slow_gain_reward > 0).float().mean()
        metrics["wm/slow_gain_reward_max"] = interest_slow_gain_reward.max()
        metrics["wm/event_replay_target_mean"] = interest_slow_gain_reward.mean()
        metrics["wm/event_replay_target_std"] = interest_slow_gain_reward.std(unbiased=False)
        metrics["wm/event_replay_target_nonzero_rate"] = (interest_slow_gain_reward > 0).float().mean()
        metrics["wm/event_replay_target_max"] = interest_slow_gain_reward.max()
        metrics["wm/event_replay_pred_mean"] = interest_slow_gain_reward.mean()
        metrics["wm/event_replay_pred_std"] = interest_slow_gain_reward.std(unbiased=False)
        metrics["wm/event_replay_pred_nonzero_rate"] = (interest_slow_gain_reward > 1e-6).float().mean()
        metrics["wm/event_replay_target_pred_corr"] = torch.ones_like(metrics["wm/event_replay_target_mean"])
        metrics["wm/event_replay_head_used"] = torch.zeros((), dtype=interest_slow_gain_reward.dtype, device=interest_slow_gain_reward.device)
        metrics["wm/high_gain_label_used"] = torch.zeros((), dtype=interest_slow_gain_reward.dtype, device=interest_slow_gain_reward.device)
        weight_entropy = -(interest_weight * (interest_weight + 1e-8).log()).sum()
        top1 = interest_weight.max()
        top5 = torch.topk(interest_weight, k=min(5, interest_weight.numel())).values.sum()
        metrics["wm/interest_weight_entropy"] = weight_entropy
        metrics["wm/interest_weight_top1"] = top1
        metrics["wm/interest_weight_top5_sum"] = top5
        metrics["wm/interest_evidence_mean"] = interest_evidence_feat.mean()
        metrics["wm/interest_residual_mag_mean"] = residual_mag_feat.mean()
        metrics["wm/interest_positive_gain_mean"] = positive_gain_feat.mean()
        ...
        metrics["intrinsic/slow_gain_reward_mean"] = interest_slow_gain_reward.mean()
        metrics["intrinsic/slow_gain_reward_nonzero_rate"] = (interest_slow_gain_reward > 1e-6).float().mean()
        metrics["intrinsic/event_replay_reward_mean"] = interest_slow_gain_reward.mean()
        metrics["intrinsic/event_replay_reward_nonzero_rate"] = (interest_slow_gain_reward > 1e-6).float().mean()
        metrics["intrinsic/slow_gain_reward_when_slow_gain_top20"] = _masked_scalar_mean(
            interest_slow_gain_reward, top20_mask
        )
        metrics["intrinsic/slow_gain_reward_when_slow_gain_not_top20"] = _masked_scalar_mean(
            interest_slow_gain_reward, not_top20_mask
        )
        ...
        metrics["wm/slow_gain_event_replay_reward_corr"] = _safe_corrcoef_tensor(
            slow_gain_reward_components["local_slow_gain"], interest_slow_gain_reward
        )
