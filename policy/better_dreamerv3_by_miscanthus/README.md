...
这个目录现在被收敛成“尽量接近原版 DreamerV3”的工程内 PyTorch 版本。

默认主线原则：
- 默认关闭 `thick_context`
- 默认关闭 `event_dynamics`
- 默认关闭 `operator_intrinsic_reward`
- 默认关闭 `action_penalty`
- 默认关闭 `slow_value`
- 默认关闭 `repval_loss`
- 默认不使用旧的 `advantage_norm`，优先走 return-range scaling

也就是说，默认训练路径尽量只保留 DreamerV3 主干：
- encoder / RSSM / decoder / reward / continue
- split KL + free nats
- symlog / two-hot reward/value
- imagined rollout actor-value training
- actor return scaling

保留这些实验模块的代码文件只是为了兼容旧配置和旧 checkpoint；它们不再构成默认主路径。

用于把参考版 DreamerV3 接入当前工程的统一策略接口。

说明：
- `policy/dreamerv3/` 是当前工作区实现，可继续开发。
- 已进入实验的冻结版本应从 `policy/versions/<version>/dreamerv3/` 加载。
- 版本绑定规则见 [policy/VERSIONING.md](/home/y/Simer/policy/VERSIONING.md)。
...
- 已补入一批 DreamerV3 风格训练细节：discrete RSSM、split KL、free nats、symlog、two-hot reward/value、actor return scaling、priority replay、train ratio
...
- value 保留标准 value head；`slow_value` 默认关闭
- replay posterior features 的 `repval_loss` 默认关闭
- actor/value 更新默认走 `return_norm` 与 actor return scaling；旧 `advantage_norm` 仅保留兼容
