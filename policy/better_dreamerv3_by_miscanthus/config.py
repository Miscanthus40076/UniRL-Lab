...
def build_dreamerv3_policy_config(policy_config: dict | None) -> tuple[DreamerV3PolicyConfig, DreamerV3CheckpointConfig]:
    ...
    # This policy is intentionally pinned close to the vanilla DreamerV3 mainline.
    # Keep parsing the old experimental sections so legacy configs don't crash, but
    # force the experimental modules off on the default execution path.
    raw.pop("thick_context", {})
    raw.pop("event_dynamics", {})
    raw.pop("action_penalty", {})
    thick_context = ThickContextConfig(enabled=False)
    event_dynamics = DreamerEventDynamicsConfig(enabled=False)
    operator_intrinsic_reward = OperatorIntrinsicRewardConfig(enabled=False)
    action_penalty = ActionPenaltyConfig(enabled=False)
    ...
    raw["aux_predict_grasp"] = False
    raw["aux_contact_num_classes"] = None
    raw["use_slow_value"] = False
    raw["repval_loss"] = False
    raw["use_advantage_norm"] = False
    raw["priority_replay"] = False
