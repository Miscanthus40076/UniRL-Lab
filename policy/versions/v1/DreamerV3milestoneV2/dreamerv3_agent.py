...
class DreamerV3Agent:
            ...
            metrics.update(
                {
                    ...
                    "dct_token_id": None,
                    "dct_effect_signature_norm": None,
                    "dct_candidate_valid": None,
                    "dct_milestone_reward": None,
                    "dct_milestone_reason": None,
                    "dct_candidate_to_seen_distance": None,
                    "dct_candidate_to_active_distance": None,
                    "dct_candidate_priority": None,
                    "dct_memory_active_size": None,
                    "dct_memory_seen_size": None,
                    "dct_milestone_reward_repeat_violation_count": None,
                }
            )
        return metrics
    ...
    def _augment_batch_with_operator_reward(
        ...
        if (
            ...
            and self.config.operator_intrinsic_reward.reward_mode == "dct_effect_milestone_v2"
            ...
            if "raw_env_reward" not in batch or "operator_milestone_reward" not in batch:
                raise RuntimeError(
                    "dct_effect_milestone_v2 requires online reward storage in replay; "
            ...
            return train_batch, {
                ...
                "dct_milestone_reward_mean": float(operator_reward.mean().detach().cpu()),
                "dct_milestone_reward_nonzero_ratio": float((operator_reward > 0).float().mean().detach().cpu()),
                "train_reward_mean": float(env_reward.mean().detach().cpu()),
                ...
                "train_reward_operator_component_mean": float((float(self.config.operator_intrinsic_reward.beta) * operator_reward).mean().detach().cpu()),
    ...
    def reset_latent(self, batch_size: int = 1):
        ...
        if self.operator_intrinsic_reward is not None:
            self.operator_intrinsic_reward.reset_lifetime_state(env_reset=True)
    ...
    def observe_event_transition(
        ...
        with torch.no_grad():
            ...
            if self.operator_intrinsic_reward is not None:
                ...
                reason_value = reward_info.get("dct_milestone_reason", [])
                reason = reason_value[0] if isinstance(reason_value, list) and reason_value else None
                operator_metrics = {
                    ...
                    "dct_token_id": float(reward_info["dct_token_id"].detach().float().mean().cpu()),
                    "dct_effect_signature_norm": float(reward_info["dct_effect_signature_norm"].detach().mean().cpu()),
                    "dct_candidate_valid": float(reward_info["dct_candidate_valid"].detach().mean().cpu()),
                    "dct_milestone_reward": float(reward_info["dct_milestone_reward"].detach().mean().cpu()),
                    "dct_milestone_reason": reason,
                    "dct_candidate_to_seen_distance": float(
                        reward_info["dct_candidate_to_seen_distance"].detach().mean().cpu()
                    ),
                    "dct_candidate_to_active_distance": float(
                        reward_info["dct_candidate_to_active_distance"].detach().mean().cpu()
                    ),
                    "dct_candidate_priority": float(reward_info["dct_candidate_priority"].detach().mean().cpu()),
                    "dct_memory_active_size": float(reward_info["dct_memory_active_size"].detach().cpu()),
                    "dct_memory_seen_size": float(reward_info["dct_memory_seen_size"].detach().cpu()),
                    "dct_milestone_reward_repeat_violation_count": float(
                        reward_info["dct_milestone_reward_repeat_violation_count"].detach().cpu()
                    ),
                }
