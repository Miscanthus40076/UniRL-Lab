...
def _build_summary(records: list[dict], global_step: int, num_episodes: int, peak_threshold: float) -> dict[str, Any]:
    ...
    dct_reward_values, dct_reward_bad = _finite_values(records, "dct_milestone_reward")
    dct_valid_values, dct_valid_bad = _finite_values(records, "dct_candidate_valid")
    dct_seen_dist_values, dct_seen_dist_bad = _finite_values(records, "dct_candidate_to_seen_distance")
    dct_active_dist_values, dct_active_dist_bad = _finite_values(records, "dct_candidate_to_active_distance")
    dct_priority_values, dct_priority_bad = _finite_values(records, "dct_candidate_priority")
    dct_active_size_values, dct_active_size_bad = _finite_values(records, "dct_memory_active_size")
    dct_seen_size_values, dct_seen_size_bad = _finite_values(records, "dct_memory_seen_size")
    dct_repeat_violation_values, dct_repeat_violation_bad = _finite_values(
        records, "dct_milestone_reward_repeat_violation_count"
    )
    ...
    dct_milestone_reward_mean = _mean_or_na(dct_reward_values)
    if dct_reward_values and total_steps > 0:
        dct_milestone_reward_nonzero_ratio = float(sum(value > 0.0 for value in dct_reward_values) / total_steps)
    else:
        dct_milestone_reward_nonzero_ratio = "NA"
    dct_milestone_active_count = _max_or_na(dct_active_size_values)
    dct_milestone_seen_count = _max_or_na(dct_seen_size_values)
    dct_milestone_reward_repeat_violation_count = _max_or_na(dct_repeat_violation_values)
    dct_milestone_candidate_distance_mean = _mean_or_na(dct_seen_dist_values)
    dct_milestone_candidate_distance_min = _min_or_na(dct_seen_dist_values)
    dct_milestone_priority_mean = _mean_or_na(dct_priority_values)
    dct_milestone_priority_min = _min_or_na(dct_priority_values)
    dct_milestone_priority_max = _max_or_na(dct_priority_values)
    ...
    return {
        ...
        "dct_milestone_reward_mean": dct_milestone_reward_mean,
        "dct_milestone_reward_nonzero_ratio": dct_milestone_reward_nonzero_ratio,
        "dct_milestone_seen_count": dct_milestone_seen_count,
        "dct_milestone_active_count": dct_milestone_active_count,
        "dct_milestone_reward_repeat_violation_count": dct_milestone_reward_repeat_violation_count,
        "dct_milestone_candidate_distance_mean": dct_milestone_candidate_distance_mean,
        "dct_milestone_candidate_distance_min": dct_milestone_candidate_distance_min,
        "dct_milestone_priority_mean": dct_milestone_priority_mean,
        "dct_milestone_priority_min": dct_milestone_priority_min,
        "dct_milestone_priority_max": dct_milestone_priority_max,
    ...
    for episode_index in range(int(gate_cfg.episodes)):
        ...
        while not done and step < int(gate_cfg.max_episode_steps):
            ...
            record = {
                ...
                "dct_effect_signature_norm": _jsonable_value(metrics.get("dct_effect_signature_norm")),
                "dct_candidate_valid": _jsonable_value(metrics.get("dct_candidate_valid")),
                "dct_milestone_reward": _jsonable_value(metrics.get("dct_milestone_reward")),
                "dct_milestone_reason": _string_or_na(metrics.get("dct_milestone_reason")),
                "dct_candidate_to_seen_distance": _jsonable_value(metrics.get("dct_candidate_to_seen_distance")),
                "dct_candidate_to_active_distance": _jsonable_value(metrics.get("dct_candidate_to_active_distance")),
                "dct_candidate_priority": _jsonable_value(metrics.get("dct_candidate_priority")),
                "dct_memory_active_size": _jsonable_value(metrics.get("dct_memory_active_size")),
                "dct_memory_seen_size": _jsonable_value(metrics.get("dct_memory_seen_size")),
                "dct_milestone_reward_repeat_violation_count": _jsonable_value(
                    metrics.get("dct_milestone_reward_repeat_violation_count")
                ),
            }
