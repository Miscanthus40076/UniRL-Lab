...
class OperatorIntrinsicRewardConfig:
    ...
    min_effect_gain: float = 0.0
    min_controllability_gain: float = 0.0
    require_effect_for_control: bool = False
    min_delta_x_norm: float = 0.0
    stale_filter_enabled: bool = False
    min_latent_delta_norm: float = 0.0
    ...
    def validate(self):
        ...
        if self.reward_mode not in {"dense_gain", "milestone", "topk_exponential"}:
            raise ValueError(
                "operator_intrinsic_reward.reward_mode must be 'dense_gain', 'milestone' or 'topk_exponential'"
            )
