...
class DreamerV3Agent:
    def __init__(self, config: DreamerV3ModelConfig, loss_config: WorldModelLossConfig | None = None):
        ...
        wm_config = DreamerV3WorldModelConfig(
            ...
            embodied_encoder=config.embodied_encoder,
        )
    ...
    def update_latent(self, obs: np.ndarray, prev_action: np.ndarray, is_first: bool = False):
        ...
        with torch.no_grad():
            embed = self.world_model.encode_step(obs_t, prev_action=action_t)["embed"]
        ...
        with torch.no_grad():
            ...
            if self.operator_intrinsic_reward is not None:
                ...
                operator_reward, reward_info = self.operator_intrinsic_reward(
                    ...
                    capacity_gate_t=(
                        None
                        if event_outputs is None or event_outputs.get("event_capacity_gate") is None
                        else event_outputs["event_capacity_gate"].unsqueeze(1)
                        if event_outputs["event_capacity_gate"].ndim == 1
                        else event_outputs["event_capacity_gate"]
                    ),
