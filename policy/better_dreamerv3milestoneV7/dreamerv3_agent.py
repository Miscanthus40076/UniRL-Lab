...
from .imagination import EXTERNAL_SLOW_GAIN_REWARD_SCALE, imagine_rollout
...
class DreamerV3Agent:
    ...
    def train_actor_value(self, batch: dict[str, torch.Tensor]) -> dict[str, float]:
        ...
        imagined = imagine_rollout(
            ...
            external_slow_gain_reward_scale=float(self.config.external_slow_gain_reward_scale),
        )
        ...
        scaled_external_slow_gain_intrinsic_mean = float(
            (external_slow_gain_intrinsics.detach() * float(self.config.external_slow_gain_reward_scale)).mean().cpu()
        )
        ...
        return {
            ...
            "actor/slow_gain_intrinsic_mean": external_slow_gain_intrinsic_mean,
            ...
            "actor/slow_gain_scaled_intrinsic_mean": scaled_external_slow_gain_intrinsic_mean,
        ...
        metrics.update(
            {
                ...
                "actor/intrinsic_to_env_abs_ratio": external_slow_gain_intrinsic_mean / (env_abs + 1e-8),
                "actor/intrinsic_to_action_penalty_ratio": external_slow_gain_intrinsic_mean
                / (action_penalty_mean + 1e-8),
            }
        )
        metadata.update(
