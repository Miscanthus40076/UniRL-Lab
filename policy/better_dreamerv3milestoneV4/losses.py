...
def summarize_slow_gain_per_step(per_step_slow_gain: torch.Tensor | None) -> dict[str, torch.Tensor]:
...
def world_model_loss(outputs: dict, batch: dict, config: WorldModelLossConfig) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    if isinstance(proprio, torch.Tensor) and isinstance(proprio_pred, torch.Tensor):
        ...
        if proprio_target.shape[-1] >= 3:
            proprio_hand_pos_mse = proprio_error[..., :3].mean()
            proprio_hand_z_mse = proprio_error[..., 2].mean()
        if proprio_target.shape[-1] >= 4:
    ...
    if event_gate is not None and ordinary_error is not None and mixed_error is not None:
        ...
        if event_capacity_gate is not None and capacity_constraint_applied:
            ...
            if config.event_sparsity_inside_capacity_only:
                event_gate_penalty_per_seq = _masked_mean_per_seq(event_gate.float(), event_capacity_gate)
            else:
                event_gate_penalty_per_seq = _reduce_except_batch(event_gate.float())
        elif context_change_mask is not None:
            event_prediction_per_seq = _masked_mean_per_seq(mixed_error.float(), context_change_mask)
            if config.event_sparsity_inside_mask_only:
                event_gate_penalty_per_seq = _masked_mean_per_seq(event_gate.float(), context_change_mask)
            else:
                event_gate_penalty_per_seq = _reduce_except_batch(event_gate.float())
        else:
        ...
        event_gate_penalty_loss = event_gate_penalty_per_seq.mean()
        event_gate_penalty_scaled = config.event_penalty * event_gate_penalty_loss
        total_per_seq = (
            total_per_seq
            + config.event_ordinary_prediction_scale * ordinary_prediction_per_seq
            + config.event_prediction_scale * event_prediction_per_seq
            + config.event_penalty * event_gate_penalty_per_seq
        )
    ...
    metrics.update(summarize_slow_gain_per_step(per_step_slow_gain))
