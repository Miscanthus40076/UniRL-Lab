...
class EventDynamicsModule(nn.Module):
    ...
    def predict(
        ...
        capacity_constraint_applied = bool(self.config.capacity_enabled and apply_capacity_constraint)
        if self.config.context_mask_enabled and context_change_mask is not None:
            ...
            if capacity_constraint_applied:
                gate_multiplier = (
                    event_gate if self.config.capacity_v2_use_sigmoid_gate_multiplier else torch.ones_like(event_gate)
                )
                mixed_pred = ordinary_pred + capacity_gate * gate_multiplier * event_residual
            else:
                mixed_pred = ordinary_pred + event_gate * event_residual
        else:
        ...
        outputs = {
            ...
            "event_capacity_constraint_applied": capacity_constraint_applied,
