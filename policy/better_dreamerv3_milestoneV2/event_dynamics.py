...
class EventDynamicsModule(nn.Module):
    ...
    def _match_rank(self, value: torch.Tensor | None, reference: torch.Tensor | None) -> torch.Tensor | None:
        if value is None or reference is None:
            return value
        # Single-step preview paths enter here with 2D feature tensors, while the
        # capacity selector normalizes logits to [B, T, 1]. Squeeze the synthetic
        # time axis back out so mixed_pred keeps the same rank as ordinary_pred.
        while value.ndim > reference.ndim and value.shape[1] == 1:
    ...
    def predict(
        ...
        if self.config.context_mask_enabled and context_change_mask is not None:
            ...
            capacity_gate = self._match_rank(capacity_gate, event_gate)
            capacity_stats = {
                key: self._match_rank(value, capacity_gate) if isinstance(value, torch.Tensor) else value
