...
说明：
- `policy/operator_token_probe/` 是当前工作区实现。
- 已进入实验的冻结版本应从 `policy/versions/<version>/operator_token_probe/` 加载。
- 独立 probe 脚本建议把输出放到具体 `exam/<exam_name>/output/...` 下；全局 `outputs/...` 只保留给脱离 exam 的临时离线诊断。
