...
class EpisodeReplayBuffer:
    ...
    def _new_episode(self) -> dict[str, object]:
        return {
            ...
            "dct_v3_bigram_reward": [],
            "dct_v3_trigram_reward": [],
            "done": [],
            "contact_mode": [],
            "is_grasping": [],
            "has_contact": True,
    ...
    def add_step(
        ...
        dct_v3_sequence_reward: float | None = None,
        dct_v3_bigram_reward: float | None = None,
        dct_v3_trigram_reward: float | None = None,
    ):
        ...
        self.current["operator_milestone_reward"].append(float(0.0 if operator_milestone_reward is None else operator_milestone_reward))
        self.current["dct_v3_sequence_reward"].append(float(0.0 if dct_v3_sequence_reward is None else dct_v3_sequence_reward))
        self.current["dct_v3_bigram_reward"].append(float(0.0 if dct_v3_bigram_reward is None else dct_v3_bigram_reward))
        self.current["dct_v3_trigram_reward"].append(float(0.0 if dct_v3_trigram_reward is None else dct_v3_trigram_reward))
    ...
    def sample_batch(
        ...
        for candidate in chosen:
            ...
            rows["operator_milestone_reward"].append(np.asarray(ep["operator_milestone_reward"][start:end], dtype=np.float32))
            rows["dct_v3_sequence_reward"].append(np.asarray(ep["dct_v3_sequence_reward"][start:end], dtype=np.float32))
            rows["dct_v3_bigram_reward"].append(np.asarray(ep["dct_v3_bigram_reward"][start:end], dtype=np.float32))
            rows["dct_v3_trigram_reward"].append(np.asarray(ep["dct_v3_trigram_reward"][start:end], dtype=np.float32))
        ...
        batch = {
            ...
            "dct_v3_sequence_reward": torch.as_tensor(
                np.asarray(rows["dct_v3_sequence_reward"]), dtype=torch.float32, device=device
            ),
            "dct_v3_bigram_reward": torch.as_tensor(
                np.asarray(rows["dct_v3_bigram_reward"]), dtype=torch.float32, device=device
            ),
            "dct_v3_trigram_reward": torch.as_tensor(
                np.asarray(rows["dct_v3_trigram_reward"]), dtype=torch.float32, device=device
            ),
