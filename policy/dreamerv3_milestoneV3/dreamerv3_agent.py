...
class DreamerV3Agent:
    ...
    def _augment_batch_with_operator_reward(
        ...
        online_reward_modes = {"dct_effect_milestone_v2", "dct_temporal_sequence_milestone_v3"}
        if (
            self.operator_intrinsic_reward is not None
            and self.config.operator_intrinsic_reward.reward_mode in online_reward_modes
            and self.config.operator_intrinsic_reward.forbid_batch_recompute_for_milestone
        ):
            if (
                self.config.operator_intrinsic_reward.reward_mode == "dct_temporal_sequence_milestone_v3"
                and not self.config.operator_intrinsic_reward.forbid_batch_recompute_for_sequence_milestone
            ):
                raise RuntimeError("DCT temporal sequence milestone V3 requires forbid_batch_recompute_for_sequence_milestone=true")
            if "raw_env_reward" not in batch or "operator_milestone_reward" not in batch:
                raise RuntimeError(
                    f"{self.config.operator_intrinsic_reward.reward_mode} requires online reward storage in replay; "
                    "missing raw_env_reward/operator_milestone_reward"
                )
            if (
                self.config.operator_intrinsic_reward.reward_mode == "dct_temporal_sequence_milestone_v3"
                and "dct_v3_sequence_reward" not in batch
            ):
                raise RuntimeError(
                    "dct_temporal_sequence_milestone_v3 requires replay field dct_v3_sequence_reward; "
                    "refusing to alias generic operator_milestone_reward as sequence reward"
                )
            raw_env_reward = batch["raw_env_reward"].detach()
            if self.config.operator_intrinsic_reward.reward_mode == "dct_temporal_sequence_milestone_v3":
                operator_reward = batch["dct_v3_sequence_reward"].detach()
                bigram_reward = batch.get("dct_v3_bigram_reward", torch.zeros_like(operator_reward)).detach()
                trigram_reward = batch.get("dct_v3_trigram_reward", torch.zeros_like(operator_reward)).detach()
            else:
                operator_reward = batch["operator_milestone_reward"].detach()
                bigram_reward = torch.zeros_like(operator_reward)
                trigram_reward = torch.zeros_like(operator_reward)
            ...
            sequence_reward = operator_reward
            return train_batch, {
                ...
                "dct_v3_sequence_reward_mean": float(sequence_reward.mean().detach().cpu()),
                "dct_v3_sequence_reward_nonzero_ratio": float((sequence_reward > 0).float().mean().detach().cpu()),
                "dct_v3_sequence_bigram_reward_mean": float(bigram_reward.mean().detach().cpu()),
                "dct_v3_sequence_trigram_reward_mean": float(trigram_reward.mean().detach().cpu()),
                "dct_v3_sequence_reward_source_is_replay_field": float(
                    self.config.operator_intrinsic_reward.reward_mode == "dct_temporal_sequence_milestone_v3"
                ),
                "dct_sequence_milestone_component_mean": float(sequence_reward.mean().detach().cpu()),
                ...
                "train_reward_operator_component_mean": float(
                    (float(self.config.operator_intrinsic_reward.beta) * operator_reward).mean().detach().cpu()
                ),
        ...
        with torch.no_grad():
            ...
            if self.operator_intrinsic_reward is not None:
                ...
                for metric_key in (
                    "dct_token_id",
                    "dct_candidate_valid",
                    "dct_effect_signature_norm",
                    "dct_v3_sequence_reward",
                    "dct_v3_sequence_bigram_reward",
                    "dct_v3_sequence_trigram_reward",
                    "dct_v3_sequence_reward_mean",
                    "dct_v3_sequence_reward_nonzero_ratio",
                    "dct_v3_sequence_reward_step_cap_hit_ratio",
                    "dct_v3_temporal_buffer_size_mean",
                    "dct_bigram_new_count",
                    "dct_bigram_seen_count",
                    "dct_bigram_active_count",
                    "dct_bigram_preemption_count",
                    "dct_bigram_rejected_seen_count",
                    "dct_bigram_rejected_active_similar_count",
                    "dct_bigram_rejected_low_priority_count",
                    "dct_bigram_repeat_violation_count",
                    "dct_trigram_new_count",
                    "dct_trigram_seen_count",
                    "dct_trigram_active_count",
                    "dct_trigram_preemption_count",
                    "dct_trigram_rejected_seen_count",
                    "dct_trigram_rejected_active_similar_count",
                    "dct_trigram_rejected_low_priority_count",
                    "dct_trigram_repeat_violation_count",
                    "dct_sequence_unique_token_ratio_mean",
                    "dct_sequence_all_same_token_ratio",
                    "dct_sequence_gap_mean",
                    "dct_sequence_span_mean",
                    "dct_sequence_candidate_distance_mean",
                    "dct_sequence_candidate_distance_min",
                    "dct_sequence_priority_mean",
                    "dct_sequence_priority_min",
                    "dct_sequence_priority_max",
                    "dct_sequence_gap",
                    "dct_sequence_span",
                    "dct_bigram_signature_norm",
                    "dct_trigram_signature_norm",
                ):
                    if metric_key in reward_info and hasattr(reward_info[metric_key], "detach"):
                        operator_metrics[metric_key] = float(reward_info[metric_key].detach().float().mean().cpu())
                reason_value = reward_info.get("dct_sequence_reason", [])
                if isinstance(reason_value, list) and reason_value:
                    operator_metrics["dct_sequence_reason"] = str(reason_value[-1])
