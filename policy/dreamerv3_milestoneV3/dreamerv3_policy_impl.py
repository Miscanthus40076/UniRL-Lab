...
class DreamerV3PolicyFolder(BasePolicy):
    ...
    def update(self, step_batch: dict) -> dict[str, object]:
        ...
        reward_mode = self.policy_cfg.operator_intrinsic_reward.reward_mode
        operator_milestone_reward = float(event_metrics.get("operator_reward") or 0.0)
        dct_v3_sequence_reward = float(event_metrics.get("dct_v3_sequence_reward") or 0.0)
        dct_v3_bigram_reward = float(event_metrics.get("dct_v3_sequence_bigram_reward") or 0.0)
        dct_v3_trigram_reward = float(event_metrics.get("dct_v3_sequence_trigram_reward") or 0.0)
        if reward_mode == "dct_temporal_sequence_milestone_v3":
            new_sequence_count = float(event_metrics.get("dct_bigram_new_count") or 0.0) + float(
                event_metrics.get("dct_trigram_new_count") or 0.0
            )
            if dct_v3_sequence_reward > 0.0 and new_sequence_count <= 0.0:
                dct_v3_sequence_reward = 0.0
                dct_v3_bigram_reward = 0.0
                dct_v3_trigram_reward = 0.0
                event_metrics["dct_v3_sequence_reward_gate_violation"] = 1.0
            else:
                event_metrics["dct_v3_sequence_reward_gate_violation"] = 0.0
            operator_milestone_reward = dct_v3_sequence_reward
        train_reward = raw_env_reward + float(self.policy_cfg.operator_intrinsic_reward.beta) * operator_milestone_reward
        ...
        self.replay.add_step(
            ...
            dct_v3_sequence_reward=dct_v3_sequence_reward,
            dct_v3_bigram_reward=dct_v3_bigram_reward,
            dct_v3_trigram_reward=dct_v3_trigram_reward,
        )
        if done:
            self.reset()
        ...
        metrics.setdefault("raw_env_reward_mean", raw_env_reward)
        metrics.setdefault("operator_milestone_reward_mean", operator_milestone_reward)
        metrics.setdefault("dct_v3_sequence_reward_mean", dct_v3_sequence_reward)
        metrics.setdefault("dct_sequence_milestone_component_mean", dct_v3_sequence_reward)
        metrics.setdefault("dct_v3_sequence_bigram_reward", dct_v3_bigram_reward)
        metrics.setdefault("dct_v3_sequence_trigram_reward", dct_v3_trigram_reward)
        metrics.setdefault("train_reward_mean", train_reward)
    ...
    def observe_transition(self, next_obs, done: bool = False, reward: float | None = None):
        ...
        return self.agent.observe_event_transition(processed, done=done, reward=reward)
