...
def world_model_loss(outputs: dict, batch: dict, config: WorldModelLossConfig) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
    ...
    if slow_gain_reward_components is not None and outputs.get("slow_gain_reward_pred") is not None:
        ...
        metrics["wm/slow_gain_reward_mean"] = slow_gain_reward_target.mean()
        metrics["wm/slow_gain_reward_std"] = slow_gain_reward_target.std(unbiased=False)
        metrics["wm/slow_gain_reward_nonzero_rate"] = (slow_gain_reward_target > 0).float().mean()
        metrics["wm/slow_gain_reward_max"] = slow_gain_reward_target.max()
        metrics["wm/event_replay_target_mean"] = slow_gain_reward_target.mean()
        metrics["wm/event_replay_target_std"] = slow_gain_reward_target.std(unbiased=False)
        metrics["wm/event_replay_target_nonzero_rate"] = (slow_gain_reward_target > 0).float().mean()
        metrics["wm/event_replay_target_max"] = slow_gain_reward_target.max()
        ...
        metrics["wm/event_replay_target_pred_corr"] = _safe_corrcoef_tensor(
            slow_gain_reward_target, slow_gain_reward_pred
        )
        ...
        metrics["intrinsic/slow_gain_reward_mean"] = slow_gain_reward_pred.mean()
        metrics["intrinsic/slow_gain_reward_nonzero_rate"] = (slow_gain_reward_pred > 1e-6).float().mean()
        metrics["intrinsic/event_replay_reward_mean"] = slow_gain_reward_pred.mean()
        metrics["intrinsic/event_replay_reward_nonzero_rate"] = (slow_gain_reward_pred > 1e-6).float().mean()
        metrics["intrinsic/slow_gain_reward_when_slow_gain_top20"] = _masked_scalar_mean(
            slow_gain_reward_pred, top20_mask
        )
        metrics["intrinsic/slow_gain_reward_when_slow_gain_not_top20"] = _masked_scalar_mean(
            slow_gain_reward_pred, not_top20_mask
        )
        ...
        metrics["wm/slow_gain_event_replay_reward_corr"] = _safe_corrcoef_tensor(
            slow_gain_reward_components["local_slow_gain"], slow_gain_reward_pred
        )
