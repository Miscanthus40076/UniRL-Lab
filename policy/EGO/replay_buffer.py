...
class EpisodeReplayBuffer:
    ...
    def _new_episode(self) -> dict[str, object]:
        return {
            ...
            "has_grasp": True,
            "priorities": [],
            "robot_fields": {key: [] for key in ROBOT_REPLAY_FIELDS},
        }

    def _append_robot_fields(self, robot_fields: dict[str, object] | None):
        robot_fields = robot_fields or {}
        for key in ROBOT_REPLAY_FIELDS:
            value = robot_fields.get(key)
            if value is None:
                self.current["robot_fields"][key].append(None)
            else:
                self.current["robot_fields"][key].append(np.asarray(value, dtype=np.float32))
    ...
    def add_step(
        ...
        robot_fields: dict[str, object] | None = None,
    ):
        ...
        self._append_robot_fields(robot_fields)
    ...
    def sample_batch(
        ...
        robot_rows = {key: [] for key in ROBOT_REPLAY_FIELDS}
        sample_refs = []
        if include_contact:
            rows["contact_mode"] = []
        if include_grasp:
        ...
        for candidate in chosen:
            ...
            robot_fields = ep.get("robot_fields", {})
            for key in ROBOT_REPLAY_FIELDS:
                values = list(robot_fields.get(key, []))[start:end]
                if len(values) == seq_len and all(value is not None for value in values):
                    robot_rows[key].append(np.asarray(values, dtype=np.float32))
        ...
        for key, values in robot_rows.items():
            if len(values) == len(chosen):
                batch[key] = torch.as_tensor(np.asarray(values), dtype=torch.float32, device=device)
