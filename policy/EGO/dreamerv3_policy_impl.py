...
from .replay_buffer import EpisodeReplayBuffer, ROBOT_REPLAY_FIELDS


OBS_PAYLOAD_KEYS = ("obs", "observation", "image", "images", "pixels", "rgb")
...
class DreamerV3PolicyFolder(BasePolicy):
    def __init__(
        ...
        if self.policy_cfg.embodied_encoder.embodied_encoder_checkpoint:
            self.policy_cfg.embodied_encoder.embodied_encoder_checkpoint = str(
                self._resolve_probe_checkpoint_path(self.policy_cfg.embodied_encoder.embodied_encoder_checkpoint)
        )
        self.processor = DreamerV3Processor(device=self.policy_cfg.device)
        self.obs_spec = self.processor.infer_observation_spec(self._policy_obs(observation_example))
    ...
    def _policy_obs(self, obs):
        if not isinstance(obs, dict):
            return obs
        for key in OBS_PAYLOAD_KEYS:
            if key in obs:
                return obs[key]
        raise ValueError(
            "Dict observation does not contain a policy observation payload. "
            f"Expected one of: {', '.join(OBS_PAYLOAD_KEYS)}"
        )

    def _extract_robot_fields(self, obs, info: dict | None, prev_action) -> dict[str, np.ndarray]:
        ...
        if isinstance(info, dict):
            sources.append(info)
        for key in ROBOT_REPLAY_FIELDS:
            if key == "prev_action":
                continue
            for source in sources:
                ...
                if array.size > 0 and np.isfinite(array).all():
                    fields[key] = array
                    break
        fields["prev_action"] = np.asarray(prev_action, dtype=np.float32).reshape(-1)
        return fields
    ...
    def act(self, obs, deterministic: bool = False):
        ...
        obs_array = self.processor.preprocess_obs(self._policy_obs(obs), self.obs_spec)
    ...
    def update(self, step_batch: dict) -> dict[str, object]:
        raw_obs = step_batch["obs"]
        raw_next_obs = step_batch["next_obs"]
        obs = self.processor.preprocess_obs(self._policy_obs(raw_obs), self.obs_spec)
        next_obs = self.processor.preprocess_obs(self._policy_obs(raw_next_obs), self.obs_spec)
        done = bool(step_batch["done"])
        info = step_batch.get("info") or {}
        robot_fields = self._extract_robot_fields(raw_obs, info, self._latent_prev_action)
        self.agent.observe_event_transition(next_obs, done=done, reward=float(step_batch["reward"]))
    ...
    def observe_transition(self, next_obs, done: bool = False, reward: float | None = None):
        processed = self.processor.preprocess_obs(self._policy_obs(next_obs), self.obs_spec)
