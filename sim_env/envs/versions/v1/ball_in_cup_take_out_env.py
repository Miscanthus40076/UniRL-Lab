...
class BallInCupTakeOutEnv(BaseEnv):
    """Ball-in-cup variant that starts with the ball inside the cup.

    Reward is 1 when the ball is moved out of the cup target region, else 0.
    """
    ...
    def __init__(
        ...
        self._init_ball_x = float(init_ball_x)
        self._init_ball_z = float(init_ball_z)
        self._ball_in_cup_module = None
        ...
        self.env = self._make_custom_env(suite)
        ...
        if self._requires_render_backend():
            print(f"[BallInCupTakeOutEnv] Using render backend '{self._selected_render_backend}'")
    ...
    def _make_custom_env(self, suite):
        control = importlib.import_module("dm_control.rl.control")
        base = importlib.import_module("dm_control.suite.base")
        ball_in_cup = self._ball_in_cup_module
        physics_cls = _make_take_out_physics_class(ball_in_cup.Physics)
        task_cls = _make_take_out_task_class(base)
        physics = physics_cls.from_xml_string(*ball_in_cup.get_model_and_assets())
        task = task_cls(random=None, init_ball_x=self._init_ball_x, init_ball_z=self._init_ball_z)
        return control.Environment(
            physics,
            task,
            time_limit=ball_in_cup._DEFAULT_TIME_LIMIT,
            control_timestep=ball_in_cup._CONTROL_TIMESTEP,
        )
    ...
    def _import_suite_with_backend_fallback(
        ...
        for index, backend in enumerate(candidates):
            ...
            try:
                ...
                self._ball_in_cup_module = importlib.import_module("dm_control.suite.ball_in_cup")
                self._selected_render_backend = backend
                if index > 0:
                    print(f"[BallInCupTakeOutEnv] Switched render backend fallback to '{backend}'")
                return suite
    ...
    def _flatten_obs(self, obs_dict):
        return np.concatenate([value.ravel() for value in obs_dict.values()]).astype(np.float32)
    ...
    def step(self, action):
        ...
        info = {
            "success": bool(self.env.physics.out_of_target()),
            "is_success": bool(self.env.physics.out_of_target()),
        }
...
class _TakeOutPhysicsMixin:
    ...
    def out_of_target(self):
        return float(not self.in_target())


def _make_take_out_physics_class(base_physics_cls):
    class TakeOutPhysics(_TakeOutPhysicsMixin, base_physics_cls):
        pass

    return TakeOutPhysics


def _make_take_out_task_class(base_module):
    class BallInCupTakeOutTask(base_module.Task):
        def __init__(self, random=None, init_ball_x=0.0, init_ball_z=0.35):
        ...
        def initialize_episode(self, physics):
            with physics.reset_context():
                ...
                physics.named.data.qpos["ball_x"] = self._init_ball_x
                physics.named.data.qpos["ball_z"] = self._init_ball_z
                physics.named.data.qvel[:] = 0.0
            physics.after_reset()
            if physics.data.ncon > 0:
                raise RuntimeError("Initial take-out state is penetrating; adjust init_ball_x/init_ball_z")
        ...
        def get_reward(self, physics):
            return physics.out_of_target()

    return BallInCupTakeOutTask
